...
public class TreeAnnotations implements java.io.Serializable {
	 ...
	 *                                           @Tag^Parent
	 *                                           ^Grandparent-&gt;_A^Tag
	 *                                           ^Parent_B^Tag^Parent and
	 *                                           finally we trim the excess _*
	 *                                           off to control the amount of
	 *                                           horizontal history
	 * 
	 * */
	public static Tree<String> processTree(Tree<String> unAnnotatedTree,
		...
		return processTree(unAnnotatedTree, nVerticalAnnotations,
				nHorizontalAnnotations, binarization, manualAnnotation, false,
				true);
	}
	...
	/**
	 * Binarize a tree with the given binarization style; e.g. head
	 * binarization, left binarization, etc.
	 * 
	 * @param tree
	 * @param binarization
	 *            The type of binarization used.
	 * @return
	 */
	...
	private static Tree<String> annotateVerticallyTwice(Tree<String> tree,
		...
		if (tree.isLeaf()) {
			...
			verticallyMarkovizatedTree = new Tree<String>(tree.getLabel()
					+ parentLabel1 + parentLabel2, children);
		}
		return verticallyMarkovizatedTree;
	}

	private static Tree<String> annotateVertically(Tree<String> tree,
			String parentLabel) {
		Tree<String> verticallyMarkovizatedTree;
		if (tree.isLeaf()) {
			verticallyMarkovizatedTree = tree;
	...
	private static Tree<String> markUnaryParents(Tree<String> tree) {
		...
		if (tree.isPreTerminal()) {
			...
			verticallyMarkovizatedTree = new Tree<String>(
					tree.getLabel() + add, children);
		}
		return verticallyMarkovizatedTree;
	}
	...
	private static Tree<String> annotateManuallyVertically(Tree<String> tree,
		...
		if (tree.isPreTerminal()) {
			...
			List<Tree<String>> children = new ArrayList<Tree<String>>();
			for (Tree<String> child : tree.getChildren()) {
				children.add(annotateManuallyVertically(child,
						"^" + tree.getLabel()));
			}
			verticallyMarkovizatedTree = new Tree<String>(tree.getLabel()
					+ parentLabel, children);
	...
	static Tree<String> leftBinarizeTree(Tree<String> tree) {
		...
		if (tree.isLeaf())
			...
			return new Tree<String>(
					label,
					Collections.singletonList(leftBinarizeTree(children.get(0))));
		}
		// otherwise, it's a binary-or-more local tree, so decompose it into a
		// sequence of binary and unary trees.
		String intermediateLabel = "@" + label + "->";
		Tree<String> intermediateTree = leftBinarizeTreeHelper(tree, 0,
				intermediateLabel);
		return new Tree<String>(label, intermediateTree.getChildren());
	}

	private static Tree<String> leftBinarizeTreeHelper(Tree<String> tree,
		...
		if (numChildrenGenerated == tree.getChildren().size() - 2) {
			children.add(leftBinarizeTree(tree.getChildren().get(
					numChildrenGenerated + 1)));
		} else if (numChildrenGenerated < tree.getChildren().size() - 2) {
			Tree<String> rightTree = leftBinarizeTreeHelper(tree,
	...
	static Tree<String> rightBinarizeTree(Tree<String> tree) {
		...
		if (tree.isLeaf())
			...
			return new Tree<String>(
					label,
					Collections.singletonList(rightBinarizeTree(children.get(0))));
		}
		// otherwise, it's a binary-or-more local tree, so decompose it into a
		// sequence of binary and unary trees.
		String intermediateLabel = "@" + label + "->";
		Tree<String> intermediateTree = rightBinarizeTreeHelper(tree,
				children.size() - 1, intermediateLabel);
		return new Tree<String>(label, intermediateTree.getChildren());
	}
	...
	private static Tree<String> rightBinarizeTreeHelper(Tree<String> tree,
		...
		if (numChildrenLeft == 1) {
			children.add(rightBinarizeTree(tree.getChildren().get(
					numChildrenLeft - 1)));
	...
	/**
	 * Binarize a tree around the head symbol, but with symbol names derived
	 * from the parent symbol, rather than the head (as in
	 * {@link headBinarizeTree}).
	 * 
	 * @param tree
	 * @return
	 */
	 ...
	 * @return
	 */
	private static Tree<String> headParentBinarizeTree(
			Binarization binarization, Tree<String> tree) {
		List<Tree<String>> children = tree.getChildren();
		if (children.size() == 0) {
			return tree;
		} else if (children.size() == 1) {
			...
			List<Tree<String>> kids = new ArrayList<Tree<String>>(1);
			kids.add(headParentBinarizeTreeHelper(binarization, tree, 0,
					children.size() - 1, headFinder.determineHead(tree), false,
					""));
			return new Tree<String>(tree.getLabel(), kids);
		}
	}
	...
	static Tree<String> headParentBinarizeTreeHelper(Binarization binarization,
			Tree<String> tree, int leftChild, int rightChild,
			Tree<String> head, boolean right, String productionHistory) {
		if (head == null)
		...
		if (!right) {
			...
			kids.add(headParentBinarizeTreeHelper(binarization, tree,
					leftChild + 1, rightChild, head, right, productionHistory
							+ "_" + child.getLabel()));
			return new Tree<String>(parentLabel, kids);
		}
		// if we're to the right of the head word
			...
			kids.add(headParentBinarizeTreeHelper(binarization, tree,
					leftChild, rightChild - 1, head, right, productionHistory
							+ "_" + child.getLabel()));
			kids.add(headParentBinarizeTree(binarization, child));
			return new Tree<String>(parentLabel, kids);
		}
	...
	public static Tree<String> removeSuperfluousNodes(Tree<String> tree) {
		...
		if (gChildren.size() != 1) {
			...
			for (int i = 0; i < gChildren.size(); i++) {
				Tree<String> cChild = removeSuperfluousNodes(tree.getChildren()
						.get(i));
				children.add(cChild);
			}
		...
		for (String node : nodesInChain) {
			if (node.equals(parent) || node.equals(cLabel))
				continue;
			Tree<String> intermediate = new Tree<String>(node,
					result.getChildren());
	...
	public static void displayUnaryChains(Tree<String> tree, String parent) {
		if (tree.getChildren().size() == 1) {
			if (!parent.equals("") && !tree.isPreTerminal())
				System.out.println("Unary chain: " + parent + " -> "
						+ tree.getLabel() + " -> "
